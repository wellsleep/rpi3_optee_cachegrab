...
enum CGState scope_attach_probe(enum cg_probe_type t, struct cache_shape *sh)
{
	int ret;
...
void scope_detach_probe(enum cg_probe_type type)
{
	if (!s.created) {
		INFO("Scope not created.");
		return;
...
struct probe *scope_get_probe(enum cg_probe_type type)
{
...
int scope_configure_probe(enum cg_probe_type type, struct probe_config *cfg)
{
	int ret;
